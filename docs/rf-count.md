...
One of the possibilities offered by RF Count is the generation of mutation map (MM) files via the ``-mm`` (or ``--mutation-map``) parameter. These files store, for each mapped read, the index of the mutations with respect to the reference transcripts, and they can be used as input for [__DRACO__](https://draco.readthedocs.io/en/latest/draco/).<br/><br/>
