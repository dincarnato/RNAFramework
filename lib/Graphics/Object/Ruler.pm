...
sub _preplotcalc {
    ...
    if ($self->{plotvalues}) {
        ...
        my ($maxheight, $height, $fontsize);
        $height = 0;
        $maxheight = max(map { $self->_textwidth($_, 10) } @{$self->{_ticks}}) || 0;
...
sub xml {
    ...
    if ($self->{plotticks} ||
        ...
        for (@{$self->{_ticks}}) {
            ...
            my $truex = maprange(@{$self->{range}}, $self->{_xpadding}, $end, $_);
            ...
            $self->tagline("text", $_, { style                => "font-size: " . $self->{fontsize} . "px; font-family: Helvetica;",
