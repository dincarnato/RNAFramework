...
sub _table {
    ...
    foreach my $data (@{$self->{_table}}) {
        ...
        for (my $i = 1; $i < @{$data}; $i++) {
            ...
            if ($i == $#{$data} &&
                (length(join("", @{$data}[1..$#{$data}])) + $self->{spacer} + $self->{indent}) > $self->{_columns}) { # When the last column contains long text
                
                my $formatted = formatlongtext($data->[$i], (sum(@{$self->{_lengths}}[1..$#{$data}-1]) + ($self->{spacer} * ($#{$data} - 1)) + $self->{indent}));
                $formatted =~ s/^\s+//;
                $table .= $formatted;
