...
sub _refine {
    ...
    for (my $i = 0; $i < @{$self->{_clusters}}; $i++) {
        ...
        $mindist = MINDIST;
        @dists = map { $self->{distfunc}->($self->{_clusters}->[$i]->[0], $self->{_centroids}->[$_]) } 0 .. $#{$self->{_centroids}};
        ...
        if ($dist < $mindist) {
            ...
            push(@{$self->{_clusters}->[$closeto]}, $self->{_clusters}->[$i]->[0]);
...
sub _kmeans {
    ...
    return (\@clusters, $centroids) if ($self->{_iterations} == $self->{maxiterations});
    ...
    return (\@clusters, $centroids) if ($pass == 1);
